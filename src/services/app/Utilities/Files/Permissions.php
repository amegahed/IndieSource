...
class Permissions
{
	...
	/**
	 * Set the writeable flag of a permissions string.
	 *
	 * @param string $permissions - the permissions string to modify.
	 * @param string $group - the group 'owner', 'group', or 'other'.
	 * @param string $value - the value 'r' or '-'
	 * @return string
	 */
	public static function setWriteable(string $permissions, string $group, string $value) {
		$string = self::digitsToString($permissions);
	...
	public static function apply(string $permissions, string $permissions2): string {
		...
		//
		$permissions2 = self::setWriteable($permissions2, 'owner', self::isWriteable($permissions, 'owner'));
		$permissions2 = self::setWriteable($permissions2, 'group', self::isWriteable($permissions, 'group'));
		$permissions2 = self::setWriteable($permissions2, 'other', self::isWriteable($permissions, 'other'));
	...
	/**
	 * Get the writeable flag of a permissions string.
	 *
	 * @param string $permissions - the permissions string to query.
	 * @param string $group - the group 'owner', 'group', or 'other'.
	 * @return bool
	 */
	public static function isWriteable(string $permissions, string $group): bool {
